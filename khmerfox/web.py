...
:root{
  --khmer-red:#e31c25;
  --khmer-red-dark:#b9131b;
  --khmer-blue:#032ea1;
  --khmer-blue-light:#1a4bc4;
  --angkor-gold:#c9a227;
  --bg:#f8f6f1;
  --surface:#ffffff;
  --surface-2:#f3f1eb;
  ...
  --text-3:#a8a29e;
  --border:#e7e5e4;
  --shadow:0 1px 3px rgba(0,0,0,0.05),0 12px 32px rgba(3,46,161,0.08);
  --radius:16px;
  --radius-sm:10px;
  --success:#15803d;
  --success-bg:#dcfce7;
...
body{
  margin:0;
  font-family:'Inter','Noto Sans Khmer',-apple-system,BlinkMacSystemFont,sans-serif;
  background:var(--bg);
  color:var(--text);
  line-height:1.5;
...
.brand-icon{
  ...
  background:linear-gradient(135deg,var(--khmer-red),var(--khmer-red-dark));
  border-radius:10px;
  display:grid;place-items:center;
  color:#fff;font-weight:700;font-size:1.1rem;
  box-shadow:0 4px 12px rgba(227,28,37,.25);
}
...
@media(min-width:960px){
  .grid-2{grid-template-columns:380px 1fr;align-items:start}
}
.card{
  background:var(--surface);
...
input:focus,select:focus{
  outline:none;
  border-color:var(--khmer-blue);
  box-shadow:0 0 0 3px rgba(3,46,161,.1);
}
...
.btn-primary{
  background:var(--khmer-red);
  color:#fff;
  box-shadow:0 4px 14px rgba(227,28,37,.25);
}
...
.progress-bar-fill{
  height:100%;
  width:0%;
  background:linear-gradient(90deg,var(--khmer-red),var(--angkor-gold));
  border-radius:999px;
...
.logs:empty::before{
  content:"Logs will appear here once scraping starts...";
  color:#78716c;
}
...
async function loadResults(){
  ...
  box.innerHTML='<div style="overflow:auto;max-height:420px;"><table><thead><tr><th>File</th><th>Type</th><th>Size</th><th>Modified</th><th></th></tr></thead><tbody>'+
    d.files.map(f=>`<tr><td>${escapeHtml(f.name)}</td><td><span class="file-type type-${f.type}">${f.type}</span></td><td class="file-size">${formatBytes(f.size)}</td><td class="file-date">${new Date(f.modified).toLocaleString()}</td><td><a class="btn btn-secondary btn-sm" href="/download/${encodeURIComponent(f.name)}" download>Download</a></td></tr>`).join('')+
    '</tbody></table></div>';
}
